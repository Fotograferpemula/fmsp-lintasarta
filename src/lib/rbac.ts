...
export type RoleName = 'superadmin' | 'manager_fms' | 'admin_pusat' | 'admin_regional' | 'admin_lokasi' | 'user';
...
export const ROLES: Record<RoleName, RoleConfig> = {
  superadmin:     { level: 1, label: 'Super Admin',     color: '#ef4444', bgColor: '#ef444420', description: 'Full system access (developer)' },
  manager_fms:    { level: 2, label: 'Manager FMS',     color: '#8b5cf6', bgColor: '#8b5cf620', description: 'Manager Facility Management Services' },
  admin_pusat:    { level: 2, label: 'Admin Pusat',     color: '#1769FF', bgColor: '#1769FF20', description: 'Administrator Pusat FMS' },
  admin_regional: { level: 3, label: 'Admin Regional',  color: '#06b6d4', bgColor: '#06b6d420', description: 'Admin Regional (multi-lokasi)' },
  admin_lokasi:   { level: 3, label: 'Admin Lokasi',    color: '#10b981', bgColor: '#10b98120', description: 'Admin Lokasi (single site)' },
  user:           { level: 4, label: 'User',            color: '#6b7280', bgColor: '#6b728020', description: 'User operasional dasar' },
};

// All valid role names
export const ROLE_NAMES: RoleName[] = ['superadmin', 'manager_fms', 'admin_pusat', 'admin_regional', 'admin_lokasi', 'user'];

// ── Permission Keys ──────────────────────────────────────
export type PermissionKey =
  | 'dashboard_view'
  | 'asset_create' | 'asset_view' | 'asset_delete'
  | 'legal_create' | 'legal_view'
  | 'wo_create' | 'wo_approve' | 'wo_delete'
  | 'user_manage'
  | 'audit_log_view'
  | 'master_data'
  | 'analytics_view'
  | 'vendor_manage' | 'vendor_view'
  | 'rab_manage' | 'rab_view'
  | 'accounting_view'
  | 'hrd_manage' | 'hrd_view'
  | 'maintenance_manage' | 'maintenance_view'
  | 'smk3_manage' | 'smk3_view'
...
const L3: RoleName[] = ['superadmin', 'manager_fms', 'admin_pusat', 'admin_regional'];
const L3L: RoleName[] = ['superadmin', 'manager_fms', 'admin_pusat', 'admin_regional', 'admin_lokasi'];
const ALL: RoleName[] = ['superadmin', 'manager_fms', 'admin_pusat', 'admin_regional', 'admin_lokasi', 'user'];
...
export const PERMISSIONS: Record<PermissionKey, RoleName[]> = {
  ...
  // Work Order
  wo_create:          ALL,
  wo_approve:         L3, // admin_regional ke atas
  wo_delete:          L2,

  // User Management
  user_manage:        L3, // admin_regional ke atas

  // Audit Log
  audit_log_view:     L3, // admin_regional (view), L2 (full)
...
export function isRegionScoped(role: string): boolean {
  return role === 'admin_regional' || role === 'admin_lokasi' || role === 'user';
}
