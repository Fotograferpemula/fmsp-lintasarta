...
import { Search, Plus, Edit2, Trash2, Box, MapPin, Archive, AlertTriangle, HelpCircle } from 'lucide-react';
                ...
                {filtered.map(item => {
                  ...
                  return (
                    <tr key={item.id} className={`transition-colors border-b ${c_table_row}`}>
                      ...
                      <td className="py-4 px-6 font-mono font-semibold text-[#3370FF]">{formatRupiah(item.qty * item.unitPrice)}</td>
          ...
          <div className={`w-full max-w-md border p-6 rounded-xl shadow-2xl space-y-6 ${c_modal}`}>
            <div className={`flex items-center justify-between border-b pb-3 ${c_border}`}>
              <div className="flex items-center gap-2">
                ...
                <button
                  type="button"
                  onClick={() => window.dispatchEvent(new CustomEvent('open-help', { detail: { key: 'inventory_add' } }))}
