...
import { Search, Plus, Edit2, Trash2, PieChart, Landmark, Percent, HelpCircle } from 'lucide-react';
...
interface RabBudget {
  ...
  spentAmount: number;
  computedSpent?: number; // Auto-calculated from linked transactions
}
...
export default function RabView({ isDark, token }: RabViewProps) {
  ...
  // Calculate statistics — use computedSpent when available
  const totalAllocated = budgets.reduce((acc, curr) => acc + curr.allocatedAmount, 0);
  const totalSpent = budgets.reduce((acc, curr) => acc + (curr.computedSpent ?? curr.spentAmount), 0);
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        <div className={`p-6 rounded-xl border ${c_card}`}>
          ...
          <h2 className="text-3xl font-extrabold mt-2 text-[#3370FF]">{formatRupiah(totalAllocated)}</h2>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left border-collapse">
              ...
              <tbody className="divide-y text-xs">
                {filtered.map(b => {
                  const actualSpent = b.computedSpent ?? b.spentAmount;
                  const utilization = b.allocatedAmount ? Math.round((actualSpent / b.allocatedAmount) * 100) : 0;
                  const remaining = b.allocatedAmount - actualSpent;
                  ...
                  return (
                    <tr key={b.id} className={`transition-colors border-b ${c_table_row}`}>
                      ...
                      <td className="py-4 px-6">
                        <div>
                          <span className="font-mono font-semibold text-red-500">{formatRupiah(actualSpent)}</span>
                          {b.computedSpent !== undefined && b.computedSpent > 0 && (
                            <span className={`block text-[9px] mt-0.5 ${c_text_sub}`}>dari transaksi</span>
                          )}
                        </div>
                      </td>
                      <td className="py-4 px-6">
                        <span className={`font-mono font-semibold ${isOverbudget ? 'text-red-500 animate-pulse' : 'text-emerald-500'}`}>
                          {formatRupiah(remaining)}
                        </span>
                        {isOverbudget && (
                          <span className="block text-[9px] text-red-400 font-bold">⚠ OVERBUDGET</span>
                        )}
                      </td>
                      <td className="py-4 px-6">
          ...
          <div className={`w-full max-w-md border p-6 rounded-xl shadow-2xl space-y-6 ${c_modal}`}>
            <div className={`flex items-center justify-between border-b pb-3 ${c_border}`}>
              <div className="flex items-center gap-2">
                ...
                <button
                  type="button"
                  onClick={() => window.dispatchEvent(new CustomEvent('open-help', { detail: { key: 'rab_add' } }))}
